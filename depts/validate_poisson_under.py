# validate_poisson_under.py
# Скрипт для валидации модели Пуассона на тотал МЕНЬШЕ 2.5
# Использует только данные, доступные до даты матча
...
class PoissonUnderValidator:
    def __init__(self, csv_path, min_team_matches=3, target_league=None, filter_odds=True):
        ...
        self.results = defaultdict(list)  # league_id -> список записей
        self.stats = defaultdict(int)

    def load_csv(self):
        df = pd.read_csv(self.csv_path, encoding='utf-8', sep=';', low_memory=False)
        df.columns = [col.strip() for col in df.columns]  # убираем лишние пробелы, но оставляем '<'

        if self.filter_odds:
            # Список колонок с коэффициентами на ТМ 2.5
            odds_cols = [
                'Avg<2.5', 'B365<2.5', 'Max<2.5', 'P<2.5',
            ...
            if existing_odds_cols:
                ...
                print(f"После фильтрации по наличию коэффициентов (ТМ 2.5) осталось {len(df)} строк")
            else:
                print("Внимание: ни одна из колонок с коэффициентами ТМ 2.5 не найдена в данных.")
        return df
    ...
    def get_poisson_over_prob_before_date(self, match):
        """Возвращает вероятность ТБ 2.5 (для последующего вычисления ТМ = 1 - over)"""
    ...
    def get_odds_from_row(self, row):
        """Извлекает коэффициент на ТМ 2.5"""
        for col in ['Avg<2.5', 'B365<2.5', 'Max<2.5', 'P<2.5',
                    'B365C<2.5', 'PC<2.5', 'MaxC<2.5', 'AvgC<2.5']:
    ...
    def process_row(self, row):
        ...
        under_prob = 1.0 - over_prob  # вероятность ТМ 2.5
        ...
        if fthg is None or ftag is None:
            ...
            return
        actual = 1 if (fthg + ftag) <= 2.5 else 0  # 1 если ТМ, 0 если ТБ
    ...
    def print_results(self, bins=None, min_matches=1):
        ...
        print("\n" + "=" * 90)
        print("АГРЕГИРОВАННАЯ КАЛИБРОВКА (ВСЕ ЛИГИ) ТМ 2.5".center(90))
        print("=" * 90)
...
def run_poisson_under_validation(csv_path='football_history_db_in_file.csv', min_team_matches=3, target_league=None, filter_odds=True):
    import django
    django.setup()
    validator = PoissonUnderValidator(csv_path, min_team_matches, target_league, filter_odds)
    validator.run()
    return validator


if __name__ == "__main__":
    print("Скрипт для калибровки ТМ 2.5 загружен.")
    print("Используйте функцию run_poisson_under_validation() для запуска.")