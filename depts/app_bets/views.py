    ...
    def get_context_data(self, **kwargs):
        ...
        try:
            ...
            for row in sheet.iter_rows(min_row=2, values_only=True):
                ...
                if not all([home_team, away_team, league_name, odds_p1, odds_p2]):
                    continue
                ...
                p1_odds = Decimal(str(odds_p1))
                p2_odds = Decimal(str(odds_p2))
                delta = Decimal('0.02')

                # Статистика для этого матча
                match_stats = {
                    'p1': {'p1': 0, 'x': 0, 'p2': 0},
                    'p2': {'p1': 0, 'x': 0, 'p2': 0},
                }

                # Матчи с кэфом П1 ±0.02
                p1_matches = Match.objects.filter(
                    league=league,
                    odds_home__gte=p1_odds - delta,
                    odds_home__lte=p1_odds + delta,
                    home_score_reg__isnull=False,
                    away_score_reg__isnull=False
                )

                for match in p1_matches:
                    if match.home_score_reg > match.away_score_reg:
                        match_stats['p1']['p1'] += 1
                    elif match.home_score_reg == match.away_score_reg:
                        match_stats['p1']['x'] += 1
                    else:
                        match_stats['p1']['p2'] += 1

                # Матчи с кэфом П2 ±0.02
                p2_matches = Match.objects.filter(
                    league=league,
                    odds_away__gte=p2_odds - delta,
                    odds_away__lte=p2_odds + delta,
                    home_score_reg__isnull=False,
                    away_score_reg__isnull=False
                )
                ...
                total_p1 = match_stats['p1']['p1'] + match_stats['p2']['p1']
                total_x = match_stats['p1']['x'] + match_stats['p2']['x']
                total_p2 = match_stats['p1']['p2'] + match_stats['p2']['p2']
                total_matches = total_p1 + total_x + total_p2

                # Считаем проценты
                if total_matches > 0:
                    p1_percent = round(total_p1 / total_matches * 100, 1)
                    x_percent = round(total_x / total_matches * 100, 1)
                    p2_percent = round(total_p2 / total_matches * 100, 1)
                else:
                    p1_percent = x_percent = p2_percent = 0
                ...
                matches_data.append({
                    ...
                    'stats': {
                        'p1_count': match_stats['p1']['p1'] + match_stats['p1']['x'] + match_stats['p1']['p2'],
                        'p2_count': match_stats['p2']['p1'] + match_stats['p2']['x'] + match_stats['p2']['p2'],
                    },
                    'result': {
                        'p1': p1_percent,
                        'x': x_percent,
            ...
            context['total_matches_in_file'] = len(matches_data)
