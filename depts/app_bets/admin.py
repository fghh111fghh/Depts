...
class MatchInline(admin.TabularInline):
    """Позволяет видеть последние игры прямо в карточке лиги или команды"""
    model = Match
    fk_name = "home_team"
    extra = 0
    fields = ('date', 'home_team', 'away_team', 'home_score_final', 'away_score_final')
...
class LeagueAdmin(admin.ModelAdmin):
    list_display = ('name', 'sport', 'country', 'display_draw_freq')
    list_filter = ('sport', 'country')
...
class MatchAdmin(admin.ModelAdmin):
    ...
    # Группировка полей в самой карточке матча
    fieldsets = (
        ('Основная информация', {
        ...
        ('Аналитика и Составы', {
            'classes': ('collapse',),  # По умолчанию скрыто
